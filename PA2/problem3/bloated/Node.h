...
public:
	HotNode();
	void copyData(OriginalNode *p);
	void setNext(HotNode *p);
	void setPrev(HotNode *p);
	HotNode * getNext();
	HotNode * getPrev();

private:
	void copyFloat(float & var, float & in);
	void copyInt(int & var, int & in);
	void copyVect(Vect_t & var, Vect_t & in);
	void copyMatrix(Matrix_t & var, Matrix_t & in);
	void copyName(char * p, const int size, char * in);

public:
	int				key;
	HotNode			*next;
	HotNode			*prev;
	struct ColdNode	*pCold;
};
